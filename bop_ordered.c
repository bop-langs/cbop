...
static addr_t
channel_for_region(addr_t region_id)
{
...
void
BOP_ordered_begin(addr_t id)
{
    if (ordered_writes.residence == NULL) {
        bop_msg(2,
                "BOP ordered port not active. BOP_ordered_region( " PRIdPTR
                " ) ignored.",
                id);
        return;
    }
    ...
    if (!in_ordered_region) {
        ...
        bop_msg(2,
                "Currently in ordered region " PRIdPTR
                ". BOP_ordered_region( " PRIdPTR " ) ignored.",
                ordered_region_id,
                id);
}
...
static void
fill_range(void *sum, mem_range_t *range)
{
...
BOP_ordered_end(addr_t id)
{
    if (in_ordered_region && ordered_region_id == id) {
        ...
        bop_msg(2,
                "No matching ordered region (%d " PRIdPTR
                "). BOP_ordered_end( " PRIdPTR " ) ignored.",
                in_ordered_region,
                ordered_region_id,
                id);
}
