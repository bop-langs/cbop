...
static void
subtree_foreach(range_node_t *node, void (*func)(mem_range_t *))
{
    if (node == NULL)
        return;
...
void
map_foreach(map_t *map, void (*func)(mem_range_t *))
{
    if (map == NULL)
        return;
...
static void
subtree_inject(range_node_t *node,
               void *sum,
...
// func must not change base/size in the range.
void
map_inject(map_t *map, void *sum, void (*func)(void *, mem_range_t *))
{
    if (map == NULL)
        return;
...
static void
to_array_block(void *_rgs, mem_range_t *range)
{
...
void
map_to_array(map_t *map, mem_range_t **ranges, unsigned *size)
{
    assert(map != NULL);
    ...
    *ranges = mspace_calloc(map->residence, map->size, sizeof(mem_range_t));
    struct ta_t tmp = {.ranges = *ranges, .size = 0};
...
static void
union_ranges(mem_range_t *r1, mem_range_t *r2, addr_t *ubase, size_t *usize)
{
...
static void
free_subtree(range_node_t *t, map_t *map)
{
    if (t == NULL)
        return;
...
static range_node_t *
absorb_left(range_node_t *node, mem_range_t *range, map_t *map)
{
    if (node == NULL)
        return NULL;
...
static range_node_t *
absorb_right(range_node_t *node, mem_range_t *range, map_t *map)
{
    if (node == NULL)
        return NULL;
    ...
    if (range->base + range->size < node->r.base) {
        ...
        addr_t rb =
            max(range->base + range->size, node->r.base + node->r.size);
...
mem_range_t *
map_add_key_val(map_t *map, addr_t key, void *obj)
{
    assert(map->is_hash);
...
void *
map_search_key(map_t *map, addr_t addr)
{
    assert(map->is_hash);
...
mem_range_t *
map_add_range(map_t *map, addr_t base, size_t size, void *obj)
{
    assert(map != NULL);
    assert(size > 0);

    bop_msg(5,
            "map %s (%p) adding range %p, size %zu, obj %p",
            map->name,
            map,
            base,
            size,
    ...
    if (has_overlap || bordering) {
        ...
        bop_msg(6,
                "map %s (%p) added range %p, size %zu, obj %p",
                map,
                map->name,
                map->root->r.base,
                map->root->r.size,
    ...
    bop_msg(6,
            "map %s (%p) added range %p, size %u, obj %p",
            map,
            map->name,
            map->root->r.base,
            map->root->r.size,
...
static char
subtree_overlaps(range_node_t *n, mem_range_t *inp, addr_t *base, size_t *size)
{
    if (n == NULL)
        return 0; /* false */
...
/* return the smallest overlap.  This is useful for enumerating all overlaps by
 * repeatedly calling this function. */

char
map_overlaps(map_t *map, mem_range_t *inp, mem_range_t *c_range)
{
    assert(map != NULL);
...
void
map_intersect(map_t *base_map, map_t *ref_map)
{
    if (!base_map || base_map->root == NULL)
        return;
...
void
map_union(map_t *base_map, map_t *ref_map)
{
    if (!ref_map || ref_map->root == NULL)
        return;
    ...
    for (i = 0; i < num; ++i) {
        ...
        if (range.rec != NULL)
            bop_msg(2,
                    "map_union warning: individual node information not "
                    "nil, which may be lost after union");
    }
...
map_t *
map_clone(map_t *source, mspace space)
{
    if (space == NULL)
        space    = source->residence;
...
void
map_subtract(map_t *base_map, map_t *ref_map)
{
    if (!ref_map || ref_map->root == NULL)
        return;
    if (!base_map || base_map->root == NULL)
        ...
        do {
            ...
            if (overlap) {
                if (orange.base > range.base)
                    map_add_range(base_map,
                                  range.base,
                                  orange.base - range.base,
                                  range.rec);
                addr_t old_bound = range.base + range.size;
...
mem_range_t *
map_contains(map_t *map, addr_t addr)
{
    assert(map != NULL);
    if (map->root == NULL)
        return NULL;
...
void
map_range_to_array(map_t *map,
                   addr_t base,
                   size_t size,
                   mem_range_t **ranges,
                   unsigned *ranges_size)
{
    assert(map != NULL);
...
static void
subtree_to_array(range_node_t *node, mem_range_t **ranges, unsigned *indx)
{
    if (node == NULL)
        return;
...
void
map_remove_node(map_t *map, addr_t node_base)
{
...
void
map_inspect(int verbose, map_t *map)
{
    int i;
    ...
    bop_msg(verbose,
            "%d range(s) in %s. (mspace %p, is_hash %d)",
            map->size,
            map->name,
            map->residence,
            map->is_hash);
    ...
    for (i = 0; i < num; ++i)
        bop_msg(verbose,
                "\t(%p - %p, page " PRIuPTR ", size %zd)",
                ranges[i].base,
                ranges[i].base + ranges[i].size,
                ranges[i].base >> PAGESIZEX,
                ranges[i].size);
...
void
init_empty_map(map_t *ret, mspace res, char *nm)
{
    ret->residence = res;
    ret->root      = NULL;
    ret->size      = 0;
...
map_t *
new_range_set(mspace res, char *nm)
{
