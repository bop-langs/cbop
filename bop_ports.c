...
void
register_port(bop_port_t *port, char *desc)
{
    ...
    if (bop_ports == NULL) {
        ...
        bop_ports = (bop_port_t **)realloc(
            bop_ports, alloc_size * 2 * sizeof(bop_port_t *));
        alloc_size *= 2;
    }
    ...
    bop_msg(
        3, "New BOP port %s (num. %d, %p) is added.", desc, num_ports, port);
}

static int
port_call_int_func(int func(void))
{
    if (func != NULL)
        return (*func)();
    else
...
int
ppr_check_correctness(void)
{
    int i, passed = TRUE;

    for (i = 0; i < num_ports && passed; i++)
        passed =
            passed && port_call_int_func(bop_ports[i]->ppr_check_correctness);
    if (!passed)
        report_conflict();
    return passed;
}
...
undy_succ_fini(void)
{
    ...
    for (i = 0; i < num_ports; i++) {
        if (bop_ports[i]->undy_succ_fini != NULL)
            bop_msg(1, "bop_ports[%d]->undy_succ_fini", i);
        port_call(bop_ports[i]->undy_succ_fini);
    }
}

void
on_exit_ordered(void)
{
    int i;

    for (i = 0; i < num_ports; i++) {
        if (bop_ports[i]->on_exit_ordered != NULL)
            port_call(bop_ports[i]->on_exit_ordered);
    }
}

void
on_enter_ordered(void)
{
    int i;

    for (i = 0; i < num_ports; i++) {
        if (bop_ports[i]->on_enter_ordered != NULL)
            port_call(bop_ports[i]->on_enter_ordered);
    }
}

/* Port code gen.  See [compiler repos]/tools/scripts/codegen.rb
 *
 * methods = ["ppr_group_init", "ppr_task_init", "spec_check", "data_commit",
 * "task_group_commit", "task_group_succ_fini", "undy_init", "undy_succ_fini"]
 *
