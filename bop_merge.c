...
void
BOP_record_read(void *addr, size_t size)
{
    if (task_status == UNDY || task_status == SEQ)
        return;
    if (ppr_pos == GAP)
        return;
    if (read_map.residence == NULL)
...
void
BOP_record_write(void *addr, size_t size)
{
    if (task_status == UNDY || task_status == SEQ)
        return;
    if (ppr_pos == GAP)
...
bop_merge_init(void)
{
    if (commit_space == NULL) {
        ...
        patches =
            (map_t *)mspace_calloc(commit_space, num_patches, sizeof(map_t));
        metacow_space = create_mspace(init_size, use_lock, is_shared_mem);
        bop_msg(
            3, "created commit (shared memory) mspace at %p", commit_space);
        ...
        if (num_patches < gs)
            patches = (map_t *)mspace_realloc(
                commit_space, patches, gs * sizeof(map_t));
        num_patches = gs;
    ...
    bop_msg(
        4,
        "bop_merge_init: (%d) patches and write union map (re-)initialized",
        num_patches);
}

static inline char
mem_range_eq(mem_range_t *r1, mem_range_t *r2)
{
...
static addr_t
check_for_overlap(map_t *my_access, char is_write_map)
{
    int i;
    ...
    if (write_union->size == 0)
        return 0;

    mem_range_t *my_ranges;
    unsigned num;
    ...
    for (i = 0; i < num; i++) {
        ...
        bop_msg(4,
                "testing range %d, from %p [" PRIdPTR "] for %zd bytes",
                i,
                mseg->base,
                mseg->base >> PAGESIZEX,
                mseg->size);
        ...
        if (same_access) {
            mem_range_t *mod_range =
                map_contains(write_union, access_overlap.base);

            bop_msg(2,
                    "Overlap with access map from %p [" PRIdPTR
                    "] for %zd bytes, %s %p-%p for %zd bytes, write union "
                    "%p-%p for %zd bytes",
                    access_overlap.base,
                    access_overlap.base >> PAGESIZEX,
                    access_overlap.size,
                    my_access->name,
                    mseg->base,
                    mseg->base + mseg->size,
                    mseg->size,
                    mod_range->base,
                    mod_range->base + mod_range->size,
                    mod_range->size);

            still_conf_free = 0;
            if (mem_range_eq(&access_overlap, mseg)) {
                mem_range_t *acc_range =
                    map_contains(write_union, access_overlap.base);
                ...
                if (mem_range_eq(acc_range, &access_overlap)) {
                    ...
                    if (same_recv) {
                        bop_msg(3,
                                "Overlap with receive set from %p [" PRIdPTR
                                "] for %zd bytes",
                                recv_overlap.base,
                                recv_overlap.base >> PAGESIZEX,
                                recv_overlap.size);

                        if (mem_range_eq(&access_overlap, &recv_overlap)) {
                            mem_range_t *recv_range =
                                map_contains(&received, recv_overlap.base);

                            if (recv_range->task == acc_range->task) {
                                bop_msg(2,
                                        "The overlap is not a conflict as the "
                                        "data was received from the last "
                                        "writer %d",
                                        recv_range->task);
                                /* save it to check for modify-after-post
                                 * conflicts */
                                ...
                                bop_msg(2,
                                        "The overlap is a post-wait ordering "
                                        "conflict since the page was received "
                                        "from %d but last written by %d",
                                        recv_range->task,
                                        acc_range->task);
                        }
                    } else
                    ...
                    bop_msg(2,
                            "The range in write_union, %p for %zu bytes, does "
                            "not equal to the overlap.  Treat it as a "
                            "conflict",
                            acc_range->base,
                            acc_range->size);
                }
                ...
                bop_msg(2,
                        "The range in %s, %p for %zu bytes, does not equal to "
                        "the overlap.  Treat it as a conflict",
                        my_access->name,
                        mseg->base,
                        mseg->size);
...
void
union_add_range(mem_range_t *range)
{
    map_add_range_from_task(
        write_union, range->base, range->size, NULL, BOP_ppr_index());
}
...
ppr_is_correct(void)
{
    ...
    if (ret != 0) {
        bop_msg(1,
                "Read-after-write conflict.  Address %p, page " PRIuPTR ".\n",
                ret,
                ret >> PAGESIZEX);
        return 0;
    }
...
static void
add_range_block(void *_sum, mem_range_t *range)
{
...
void
create_patch(map_t *patch, map_t *change_set, mspace space)
{
    init_empty_map(patch, space, "patch");

    struct patch_inject_t sum = {.patch = patch, .total = 0};

    map_inject(change_set, &sum, add_range_block);

    bop_stats.data_copied += sum.total;
    bop_msg(2,
            "Created a patch with %d range(s) for a total of %d bytes.",
            patch->size,
            sum.total);
}

static void
apply_range_block(void *_sum, mem_range_t *range)
{
    unsigned *sum = (unsigned *)_sum;

    memcpy((void *)range->base, range->rec, range->size);
    bop_msg(6,
            "Copy-in starting %p (page " PRIdPTR ") for %zd bytes.",
            range->base,
            range->base >> PAGESIZEX,
            range->size);
    *sum += range->size;
}

void
apply_patch(map_t *patch)
{
    unsigned sum = 0;
...
static void
free_range_data(void *space, mem_range_t *range)
{
    mspace_free((mspace)space, range->rec); /* cleaning up */
}

void
clear_patch(map_t *patch)
{
...
bop_port_t bop_merge_port = {.ppr_group_init        = bop_merge_init,
