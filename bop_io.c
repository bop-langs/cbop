...
static void
init_buffer(output_buffer *buf, char *data, size_t size)
{
    assert(data != NULL);
    assert(size >= 1);
    buf->alloc   = size;
...
io_group_init(void)
{
    ...
    output_buffers =
        mspace_calloc(io_mspace, num_buffers, sizeof(output_buffer));
    assert(output_buffers != NULL);
...
bop_port_t bop_io_port = {.ppr_group_init       = io_group_init,
...
int
terminal_helper(char *msg, int gpid)
{
...
bop_terminal_to_monitor()
{
    ...
    return terminal_helper("Couldn't send terminal to monitor process",
                           monitor_process_id);
}

int
bop_terminal_to_workers()
{
    if (!given_to_workers)
        return terminal_helper("Couldn't send terminal to worker processes",
                               monitor_group);
    return 0;
}
