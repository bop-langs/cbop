...
bop_msg_va(int level, const char *msg, va_list v)
{
    if (bop_verbose >= level) {
        ...
        switch (task_status) {
        case UNDY:
            fprintf(stderr, "Undy-(idx %d%s): ", pidx, pos);
            break;
        case MAIN:
            fprintf(stderr, "Main-%d(idx %d%s): ", spec_order, pidx, pos);
            break;
        case SEQ:
            fprintf(stderr, "Seq-(idx %d%s): ", pidx, pos);
            break;
        case SPEC:
            fprintf(stderr, "Spec-%d(idx %d%s): ", spec_order, pidx, pos);
            break;
        }
        struct timeval tv;
...
int
get_int_from_env(const char *env, int min, int max, int def)
{
    char *cval;
    ...
    if (ival > max)
        ival = max;
    bop_msg(2,
            "Variable %s is set as %d based on env ([%d, %d]).",
            env,
            ival,
            min,
            max);
