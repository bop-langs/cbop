...
#define SEQUENTIAL()                                                   \
    (malloc_panic || bop_mode == SERIAL || BOP_task_status() == SEQ || \
     BOP_task_status() == UNDY)
...
// Header info
static header *headers[DM_NUM_CLASSES] =
    {[0 ... DM_NUM_CLASSES - 1] = NULL}; // Current heads of free lists

static header *ends[DM_NUM_CLASSES] = {[0 ... DM_NUM_CLASSES - 1] =
                                           NULL}; // End of lists in PPR region

static header *freedlist[DM_NUM_CLASSES] =
    {[0 ... DM_NUM_CLASSES - 1] =
         NULL}; // List of items freed during PPR-mode.

static header *large_free_list = NULL;

static header *allocated_lists[DM_NUM_CLASSES] = {[0 ... DM_NUM_CLASSES - 1] =
                                                      NULL}; // List of items
...
free_now(header *);
static inline bool
list_contains(header *list, header *item);
static bool
remove_from_alloc_list(header *);
static inline void
add_next_list(header **, header *);
static inline void
add_freed_list(header *);
static inline header *
dm_split(int which, int larger);
static inline int
index_bigger(int);
static inline size_t
align(size_t size, size_t align);
static inline void
get_lock();
static inline void
release_lock();
 ...
 * means SIZE class 1. The original implementation had an array that had these
 * values*/
const FORCE_INLINE size_t
size_of_klass(int klass_index)
{
    return SIZE_C(klass_index + 1);
}

const FORCE_INLINE int
goal_blocks(int klass)
{
    switch (klass) {
    case 0 ... 3:
...
/**Get the index corresponding to the given size. If size > MAX_SIZE, then
 * return -1*/
static inline int
get_index(size_t size)
{
    ...
    if (index == -1 ||
        size_of_klass(index) <
            size) // this is how it gets rounded up only if needed
        index++;
    bop_assert(index >= 0 && index < DM_NUM_CLASSES);
    bop_assert(size_of_klass(index) >=
               size); // This size class is large enough
    bop_assert(index == 0 ||
               size_of_klass(index - 1) <
                   size); // Using the minimal valid size class
    return index;
}
...
initialize_group()
{
    ...
    for (ind = 0; ind < DM_NUM_CLASSES; ind++) {
        ...
        // bop_msg(3, "DM malloc task %d header[%d] = %p", spec_order, ind,
        // headers[ind]);
    }
#ifndef NDEBUG
    for (ind = 0; ind < DM_NUM_CLASSES; ind++) {
...
*/
// NOTE: only the heads should be promised. The payloads should be the job of
// the caller?
void
malloc_promise()
{
    ...
    for (i = 0; i < DM_NUM_CLASSES; i++) {
        for (head = allocated_lists[i]; head != NULL;
             head = CAST_H(head->next)) {
...
dm_malloc_undy_init()
{
    return; // TODO this method should actually work, but it causes Travis to
    // fail.
    ...
    for (ind = 0; ind < DM_NUM_CLASSES; ind++) {
        for (current = freedlist[ind]; current != NULL;
             current = CAST_H(current->next)) {
            dm_free(PAYLOAD(current));
        }
        freedlist[ind] = NULL;
    }
    for (current = large_free_list; current != NULL;
         current = CAST_H(current->next)) {
        dm_free(PAYLOAD(current));
    }
    for (ind = 0; ind < DM_NUM_CLASSES; ind++) {
...
grow(const int tasks)
{
    ...
    for (class_index = 0; class_index < DM_NUM_CLASSES; class_index++) {
        blocks_left =
            tasks * goal_counts_arr[class_index] - counts[class_index];
    ...
    for (class_index = 0; class_index < DM_NUM_CLASSES; class_index++) {
        ...
        space_head = sys_calloc(blocks[class_index],
                                size); // system malloc, use byte-sized type
        bop_assert(blocks[class_index] == 0 ||
                   space_head != NULL); // Ran out of sys memory

        for (current_block = 0; current_block < blocks[class_index];
             current_block++) {
            // Set up this space
...
// Make sure the last B in the head is writtable
            ...
            if (grow_count > 1) { /** Utilization tests */
                ...
                bop_assert(
                    current_block == 0 ||
                    headers[class_index] ==
                        (header *)&space_head[(current_block - 1) * size]);
                // calloc'd return & seperate from previous head
                bop_assert(*debug_ptr == (char)0);
                // last byte is readable
                bop_assert(current_block == blocks[class_index] - 1 ||
                           debug_ptr[size - 1] == (char)0);
                // Adjacent in memory to previous block
                bop_assert(
                    current_block == 0 ||
                    debug_ptr[-1] ==
                        'x'); // Previous byte is the last of the previous
                debug_ptr[size - 1] = 'x'; // This is an array, so -1 since the
                // highest array index is -1 than
                // length
            }
#endif
...
// Post-size class division checks
        ...
        if (grow_count > 1) {
            ...
            for (current = CAST_H(headers[class_index]->next); current;
                 current = CAST_H(current->next)) {
...
// Get the head of the free list. This uses get_index and additional logic for
// PPR execution
static inline header *
get_header(size_t size, int *which)
{
    header *found = NULL;
    int temp      = -1;
    ...
    if (size > MAX_SIZE) {
        ...
        if (!SEQUENTIAL() && found != NULL) {
            // This will be useless in sequential mode, and useless if found ==
            // NULL
            if (ends[temp] != NULL && CAST_UH(found) == ends[temp]->next) {
                bop_msg(3,
                        "Something may have gone wrong: value of ends[which]: "
                        "%p\t value of which: %d",
                        ends[temp],
                        temp);
                found = NULL;
...
int alist_added = 0;
extern void
BOP_malloc_rescue(char *, size_t);

// BOP-safe malloc implementation based off of size classes.
void *
dm_malloc(const size_t size)
{
...
malloc_begin:
    ...
    if (block == NULL) {
        ...
        if (alloc_size > MAX_SIZE) {
            if (SEQUENTIAL()) { // WORKING!
                ...
                if (block == NULL) {
                    // ERROR: ran out of system memory. malloc rescue won't
                    // help
                    release_lock();
                    return NULL;
                }
                ...
                // Not SEQUENTIAL(), and allocating a too-large block. Might be
                // able to rescue
                BOP_malloc_rescue("Large allocation in PPR", alloc_size);
                goto malloc_begin; // Try again
            }
        } else if (which < DM_NUM_CLASSES - 1 &&
                   (bigger = index_bigger(which)) != -1) {
    ...
    if (!SEQUENTIAL()) {
        bop_assert(which !=
                   -1); // Valid because -1 == too large, can't do in PPR
...
print_headers()
{
    ...
    for (ind = 0; ind < DM_NUM_CLASSES; ind++) {
        bop_msg(1,
                "headers[%d] = %p get_header = %p",
                ind,
                headers[ind],
                get_header(size_of_klass(ind), NULL));
    }
}

// Compute the index of the next lagest index > which st the index has a
// non-null headers
static inline int
index_bigger(int which)
{
    if (which == -1)
        return -1;
    which++;
...
dm_split(int which, int larger)
{
    ...
    header *split =
        CAST_H((CHARP(block) + size_of_klass(which))); // Cut in half
    ...
    while (which < larger) {
        // Update the headers
        split = CAST_H(
            (CHARP(split) + size_of_klass(which - 1))); // Which - 1 since only
...
void *
dm_calloc(size_t n, size_t size)
{
    header *head;
...
// Reallocator: use sytem realloc with large->large sizes in SEQUENTIAL() mode.
// Otherwise use standard realloc implementation
void *
dm_realloc(void *ptr, size_t gsize)
{
    ...
    if (new_index != -1 && size_of_klass(new_index) <= old_head->blocksize) {
    ...
    } else if (SEQUENTIAL() && old_head->blocksize > MAX_SIZE &&
               new_size > MAX_SIZE) {
        ...
        if (new_payload == NULL) {
            bop_msg(1,
                    "Unable to reallocate %p(size %u) to new size %u",
                    ptr,
                    old_size,
                    new_size);
            return NULL;
        ...
        // Copy the data
        size_t copy_size =
            MIN(old_size, new_size) - HSIZE; // block sizes include the header!

        assert(copy_size != new_size - HSIZE);
        bop_assert(HEADER(new_payload)->blocksize >=
                   (copy_size + HSIZE)); // Check dm_malloc gave enough space
...
// Free a(regular or huge) block now. all saftey checks must be done before
// calling this function
static inline void
free_now(header *head)
{
    int which;
    ...
    bop_assert(size >= HSIZE &&
               size ==
                   ALIGN(size)); // Size is aligned, ie right value was written
...
inline size_t
dm_malloc_usable_size(void *ptr)
{
    if (ptr == NULL)
        return 0;
    header *free_header = HEADER(ptr);
...
/*malloc library utility functions: utility functions, debugging, list
 * management etc */
static bool
remove_from_alloc_list(header *val)
{
    // Remove val from the list
    header *current, *prev = NULL;
    const int index = get_index(val->blocksize);

    for (current = allocated_lists[index]; current;
         prev = current, current = CAST_H(current->next)) {
        if (current == val) {
            if (prev == NULL) {
...
static inline bool
list_contains(header *list, header *search_value)
{
...
static inline void
add_next_list(header **list_head, header *item)
{
    added_n++;
    if (!SEQUENTIAL())
        nseq_added_n++;
...
dm_print_info(void)
{
#ifndef NDEBUG
    setlocale(LC_ALL, "");
    int *counts = count_lists(true); // We don't actually have the lock, but

    // don't care about thread saftey here
    int i;
    int GROW_S = 0;
    ...
    for (i = 0; i < DM_NUM_CLASSES; i++) {
        printf("\tSplit to give class %d(%'lu B) %d times. It was given %d "
               "heads\n",
               i + 1,
               size_of_klass(i),
               split_attempts[i],
...
#else
    printf("dm malloc not compiled in debug mode. Recompile without NDEBUG "
           "defined to keep track of debug information.\n");
#endif
}

bop_port_t bop_alloc_port = {.ppr_group_init    = carve,
