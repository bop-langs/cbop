    ...
    do {
        if (count == SUBSTRING_SIZE) {
            ...
            ret->string[substring] =
                malloc((SUBSTRING_SIZE + 1) * sizeof(char));
        }
...
void
BOP_print_string(BOP_string *str)
{
    int i = 0;
...
BOP_string *
BOP_replace(BOP_string *original, char *find, char *replace)
{
    int i           = 0;
    int len         = strlen(find);
    ...
    while (original->string[i] != NULL) {
        ...
        if (num_matches != 0 || count != 0) {
            ...
            if (count != 0) {
                ...
                if (original->string[i + 1] != NULL) {
                    ...
                    if (leftover > 0) {
                        ...
                        BOP_promise(original->string[i + 1],
                                    strlen(original->string[i + 1] + 1) *
                                        sizeof(char));
                        while ((original->string[i + 1][jj] =
                                    original->string[i + 1][jj + leftover]) !=
                               '\0') {
                            jj++;
                        }
                    }
            ...
            if (new_substring_size > old_substring_size) {
                ret = malloc((new_substring_size * SUBSTRING_SIZE + 1) *
                             sizeof(char));
            }

            /* If a match was found between substrings, it gets added to the
             * tail of
             * the first substring */
            if (first_copy) {
                int ii;
            ...
            /* Next, the substring is iterated through and find/replace is
             * performed.
             * When find/replace occurs in this code, it ends up iterating
             * through the
             * string twice, once to count the number of finds, and once to
             * actually do
             * the replaces. If this were not the case, then some kind of data
             * structure
             * would be needed to keep track of these finds. But then, all
             * characters after
             * a replace would still need to be iterated over. So overall, I
             * don't think
             * any speed would be gained by doing that and it would add some
             * complexity */
            ...
            if (jump_size > 0 && num_matches != 0) {
                ...
                /* The search is performed backwards if new the substring is
                 * larger than the
                ...
                for (; j >= 0; j--) {
                    if (original->string[i][j] == find[r_count - 1]) {
                        ...
                        if (r_count == 0) {
                            ...
                            r_count = len;
                            if (jump == 0 &&
                                new_substring_size < old_substring_size) {
                                break;
                            }
                        } else {
            ...
            BOP_promise(
                original->string[i],
                ((new_substring_size * SUBSTRING_SIZE + 1) * sizeof(char)));
            num_matches = 0;
        }
        BOP_ppr_end(1);
        i++;
...
char *
BOP_get_string(BOP_string *bstr)
{
    return NULL;
}
...
main(int argc, char *argv[])
{
    ...
    /*BOP_string* b = BOP_create_string("My name is Inigo Montoya. You killed
     * my father. Prepare to name.");
     * BOP_print_string(b);
