...
silent_drop(void)
{
    ...
    bop_msg(2,
            "same datum from two tasks received in reverse sequential order");
    x           = 0;
    spec_order  = 2;
    ppr_index   = 2;
    task_status = SPEC;
...
correctness_check(void)
{
    ...
    channel_post(&z);
    // z = 2;  // uncomment to test modify-after-post conflict handling, next
    // ppr will abort
    // bop_mode = PARALLEL; // uncomment to test modify-after-post conflict
    // handling
...
interface_tests(void)
{
    ...
    bop_msg(2,
            "test 1: two pprs, pass one number, one successful and two "
            "failed posts");
    task_status = SEQ;
    ppr_group_init();
    spec_order  = 0;
    ...
    channel_post(21); /* resource conflict (depending on the number of channels
                       * allocated */
