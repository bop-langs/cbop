...
static void
check(map_t *map, unsigned nodes, addr_t bytes)
{
    // map_inspect( 1, map, "map" );
    assert(map->size == nodes);
    assert(map_size_in_bytes(map) == bytes);
    bop_msg(
        1, "check map passes, %u node(s), " PRIdPTR " byte(s)", nodes, bytes);
}

static void
check_range(mem_range_t *x, addr_t base, size_t size)
{
    assert(x->base == base);
    assert(x->size == size);
}
...
test1(mspace space)
{
    ...
    map_add_range(vms, (addr_t)vm_data, num_segments * chunksize, NULL);
    map_add_range(vms, (addr_t)vm_data2, num_segments * chunksize, NULL);
