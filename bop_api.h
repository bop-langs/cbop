...
#define bop_debug(x, ...) \
    bop_msg(1, "%s:%d " x "", __FILE__, __LINE__, ##__VA_ARGS__);
#else
#define bop_debug(...)
#endif
void
error_alert_monitor(void);
...
typedef enum { PPR = 0, GAP, PSEQ } ppr_pos_t;

typedef enum { SERIAL = 0, PARALLEL } bop_mode_t;

task_status_t
BOP_task_status(void);
ppr_pos_t
BOP_ppr_pos(void);
bop_mode_t
BOP_mode(void);
...
int
BOP_get_group_size(void);
int
BOP_get_verbose(void);

void
BOP_set_group_size(int sz);
void
BOP_set_verbose(int x);
...
typedef void
monitor_t(void *, size_t);

/* Called by a speculation process in case of error. */
void
BOP_abort_spec(const char *msg, ...) __attribute__((format(printf, 1, 2)));
void
BOP_abort_next_spec(char *msg, ...) __attribute__((format(printf, 1, 2)));

/* FILE I/O */
int
BOP_printf(const char *format, ...);
int
BOP_fprintf(FILE *stream, const char *format, ...);

/* Check if addr has been read/written. Return the memory range;
 * otherwise NULL */
mem_range_t *
BOP_check_read(void *addr);
mem_range_t *
BOP_check_write(void *addr);
mem_range_t *
BOP_check_access(void *addr);
...
bop_get_verbose(void);
void
bop_msg(int level, const char *msg, ...);
...
/* For BOP_malloc */
void *
_BOP_malloc(size_t sz, char *file, unsigned line);
void *
_BOP_calloc(size_t n_elements, size_t elem_size, char *file, unsigned line);
void
_BOP_free(void *mem, char *file, unsigned line);
void *
_BOP_realloc(void *mem, size_t newsize, char *file, unsigned line);
