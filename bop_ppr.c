...
end_clean(void); // exit if children errored or call abort
static int
cleanup_children(void); // returns the value that end_clean would call with

// _exit (or 0 if would have aborted)
void
SigBopExit(int signo);
static void
block_wait(void);
static void
unblock_wait(void);
...
_ppr_group_init(void)
{
    ...
    ppr_group_init();
    task_status =
        MAIN; // at the end so monitoring is disabled until this point
}
...
_ppr_task_init(void)
{
    ...
    bop_msg(2,
            "ppr task %d (%d total) starts. pgrp = %d",
            spec_order,
            BOP_get_group_size(),
            getpgrp());
...
_BOP_ppr_begin(int id)
{
    ...
    case SPEC:
        if (task_status == SPEC && old_pos != GAP) {
            bop_msg(2,
                    "Nested begin PPR (region %d inside region %d) ignored",
                    id,
                    ppr_static_id);
            return 0;
        }
...
void
BOP_malloc_rescue(char *msg, size_t size)
{
    bop_msg(2,
            "Malloc rescue begin. aligned size & header: %u Failure: %s",
            size,
            msg);
    // print_headers();
    if (task_status == SEQ || task_status == UNDY || bop_mode == SERIAL) {
        bop_msg(1, "ERROR. Malloc failed while logically sequential");
    } else if (task_status == MAIN ||
               (task_status == SPEC && spec_order == 0)) {
        bop_msg(3,
                "Changing pid %d (mode %s)",
                getpid(),
                task_status == MAIN ? "Main" : "SPEC");
        // Hangs here... io_on_malloc_rescue(); //before anything changes, but
...
static inline void
abort_printf(const char *abrt_msg, const char *usr_msg, va_list argptr)
{
    char buffer[strlen(usr_msg) + strlen(abrt_msg) + 2];
    size_t size = sizeof(buffer);
    size_t written =
        snprintf(buffer, sizeof(buffer), "%s %s", abrt_msg, usr_msg);
...
post_ppr_undy(void)
{
    ...
    kill(
        -monitor_group,
        SIGUSR1); // TODO why is this here????? main requires a special signal?
...
spawn_undy(void)
{
    ...
    case 0:
        ...
        assert(getpgrp() == -monitor_group);
        bop_msg(3,
                "Understudy starts pid %d pgrp %d parent: %d",
                getpid(),
                getpgrp(),
                caller);
...
_BOP_group_over(int id)
{
    if (ppr_static_id != id) {
        ...
        bop_msg(3,
                "Valid state while hitting BOP_group_over. Allowing to "
                "pass barrier");
    }
}
...
_BOP_ppr_end(int id)
{
    ...
    if (ppr_pos == GAP || ppr_static_id != id) {
        bop_msg(4,
                "Unmatched end PPR (region %d in/after region %d) ignored",
                id,
                ppr_static_id);
        return;
    }
 ...
 * to Undy.
 *
 * In addition, the monitor/timing process begins it's clean up process when it
 * recieves SigUsr1.
 *
 * When the monitor process recieves SIGUSR2, it immediately kills all running
 * processes using SIGKILL,
 * and exits in error.
 * All processes have the same SIGUSR1 and SIGUSR2 handlers.
 *
 */
void
MonitorInteruptFwd(int signo)
{
    bop_msg(1,
            "forwarding signal '%s' to children of pgrp %d",
            strsignal(signo),
            monitor_group);
...
ErrorKillAll(int signo)
{
    ...
    /**Horrible things are happening. Go to SEQ mode so malloc won't have
     * issues*/
    malloc_panic = true;
...
void
SigUsr1(int signo, siginfo_t *siginfo, ucontext_t *cntxt)
{
    assert(SIGUSR1 == signo);

    if (task_status == UNDY) {
        bop_msg(3,
                "Understudy concedes the race (sender pid %d)",
                siginfo->si_pid);
    ...
    if (task_status == SPEC || task_status == MAIN) {
        if (spec_order == partial_group_get_size() - 1)
            return;
        bop_msg(3,
                "Quiting after the last spec succeeds (sender pid %d)",
                siginfo->si_pid);
        end_clean(); // wait for children. doesn't return
    }
    if (getpid() == monitor_process_id) {
        bop_msg(3,
                "Monitor process setting is_monitoring to false. Sender %d",
                siginfo->si_pid);
        is_monitoring = false;
    }
}
...
void
SigUsr2(int signo, siginfo_t *siginfo, ucontext_t *cntxt)
{
    assert(SIGUSR2 == signo);
    assert(cntxt);
    if (getpid() == monitor_process_id) {
        bop_msg(1,
                "Monitor process exiting main loop because of SIGUSR2 (error)",
                siginfo->si_pid);
        is_monitoring = false;
        errored       = true;
...
int
report_child(pid_t child, int status)
{
    if (child == -1 || !child)
        return DEF_EXIT;
    char *msg;
    ...
    // Have it return true if the signal that kill the process is either
    // SIGABRT or SIGSEGV
...
wait_process()
{
    ...
    assert(monitor_group !=
           0); // was actually written by the PIPE before this call
    bop_msg(3,
            "Monitoring pg %d from pid %d (group %d)",
            monitor_group,
            getpid(),
            getpgrp());
    ...
    while (((child = waitpid(monitor_group, &status, WUNTRACED)) != -1)) {
        my_exit =
            my_exit ||
            report_child(child, status); // we only care about zero v. not-zero
    }
    unblock_wait();
    if (errno && errno != ECHILD) {
        perror(
            "Error in wait_process. errno != ECHILD. Monitor process endings");
        _exit(EXIT_FAILURE);
    }
    my_exit = my_exit || errored;
    my_exit = my_exit ? 1 : 0;
    bop_msg(1,
            "Monitoring process %d ending with exit value %d",
            getpid(),
            my_exit);
    msg_destroy();
    kill(monitor_group, SIGKILL); // ensure that everything is killed.
    // Once the monitor process is done, everything should have already
    // terminated
    _exit(my_exit);
}
...
cleanup_children()
{
    ...
    while ((child = waitpid(monitor_group, &status, WUNTRACED)) !=
           -1) { // any child
...
void __attribute__((constructor)) BOP_init(void)
{
    ...
    if (bop_mode != SERIAL) {
        ...
        case 0:
            ...
            break;
        default:
            monitor_group = -fd; // child will set up its monitor_group
            // variable
            OWN_GROUP(); // monitoring process gets its own group, useful for
            // ruby test suite

            bop_msg(1, "Child proc id: %d pgrd %d", getpid(), getpgrp());
            // forward SIGINT to children/monitor group
            signal(SIGINT,
                   MonitorInteruptFwd); // sigint gets forwarded to children
            is_monitoring = true; // the real monitor process is the only one
            // to actually loop
...
BOP_fini(void)
{
    ...
    case SPEC:
        ...
        kill(0,
             SIGUSR2); // send SIGUSR to spec group but not ourself-> own group
    ...
    bop_msg(1,
            "\n***BOP Report***\n The total run time is %.2lf seconds.  There "
            "were %d ppr tasks, %d executed speculatively and %d "
            "non-speculatively (%d by main and %d by understudy). pid %d\n",
            bop_end_time - bop_stats.start_time,
            ppr_index,
            bop_stats.num_by_spec,
            bop_stats.num_by_undy + bop_stats.num_by_main,
            bop_stats.num_by_main,
            bop_stats.num_by_undy,
            getpid());

    bop_msg(3,
            "A total of %d bytes are copied during the commit and %d bytes "
            "are posted during parallel execution. The speculation group size "
            "is %d.\n\n",
            bop_stats.data_copied,
            bop_stats.data_posted,
            BOP_get_group_size());
...
kill_monitor:
    ...
    bop_msg(3,
            "Sending shutdown signal to monitor process %d from pid %d",
            monitor_process_id,
            getpid());
