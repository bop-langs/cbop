...
postwait_init(void)
{
    if (pw_space == NULL) {
        ...
        switch_board.channels = (channel_t *)mspace_calloc(
            pw_space, switch_board.size, sizeof(channel_t));
        assert(metacow_space !=
               NULL); /* bop_merge_port.ppr_group_init needed */
        ...
        if (switch_board.size < newsize) {
            ...
            switch_board.channels = (channel_t *)mspace_realloc(
                pw_space,
    ...
    memset(switch_board.channels, 0, switch_board.size * sizeof(channel_t));
    bop_lock_clear(&switch_board.lock);

    bop_msg(
        4, "postwait_init: (%d) channels (re-)initialized", switch_board.size);
}

static int
has_local_channel(addr_t id)
{
    assert(id > 0);
    if (local_ch_id != 0 && local_ch_id != id) {
        bop_msg(1,
                "channel fill/post for " PRIdPTR
                " is ignored (currently " PRIdPTR
                "). use channel_local_reset to clear the previous record or "
                "wait for next BOP version to allow working on multiple "
...
void
channel_fill(addr_t id, addr_t base, unsigned size)
{
    if (task_status == SEQ || task_status == UNDY)
        return;
    if (!has_local_channel(id))
        return;
    ...
    bop_msg(4,
            "channel fill : map_add_range %p, %p, %.0f, %d",
            &local_ch_data,
            base,
            *((double *)base),
            size);
...
extern void
map_subtract(map_t *, map_t *);

void
channel_post(addr_t id)
{
    if (task_status == SEQ || task_status == UNDY)
        return;
    if (!has_local_channel(id))
    ...
    if (ch->id != 0 && ch->id != id) {
        bop_msg(1,
                "channel " PRIdPTR " conflict with " PRIdPTR
                " (task %u), fail to allocate",
                id,
                ch->id,
                ch->sender);
    ...
    if (ch->is_posted) {
        bop_msg(1,
                "Posting to posted channel " PRIdPTR " (by task %u), ignored",
                ch->id,
                ch->sender);
        bop_lock_release(&switch_board.lock);
        return;
...
static void
set_chain(channel_t *ch, channel_t *fst, channel_t *nxt, char posted)
{
    ch->cc_first  = fst;
    ch->cc_next   = nxt;
    ch->is_posted = posted;
...
static void
connect_chain(channel_t *head, channel_t *new_ch, char posted)
{
...
void
channel_chain(addr_t id1, addr_t id2)
{
    if (task_status == SEQ || task_status == UNDY)
        return;
    bop_lock_acquire(&switch_board.lock);
    ...
    if (ch1 == NULL || ch2 == NULL) {
        bop_msg(1,
                "chaining of " PRIdPTR " and " PRIdPTR
                " failed for lack of resource",
                id1,
                id2);
        bop_lock_release(&switch_board.lock);
        return;
...
range_node_t *
new_node(mspace msp, addr_t base, size_t size, void *obj);
range_node_t *
splay(range_node_t *t, addr_t key);
char
overlap(mem_range_t *b, mem_range_t *r, addr_t *base, size_t *size);

/* Split the range when needed, which will return a non-nil range node
 * leftover */
static range_node_t *
range_remove_overlap_return_leftover(mem_range_t *range,
                                     mspace lospace,
                                     addr_t obase,
                                     size_t osize)
{
    ...
    if (range->base + range->size > obase + osize) {
        range_node_t *n = new_node(lospace,
                                   obase + osize,
...
static char *
data_clone(mspace space, char *src, size_t size)
{
...
mem_range_t *
map_add_range_from_task(
    map_t *map, addr_t base, size_t size, void *obj, unsigned ppr_id)
{
    assert(map != NULL);
    assert(size > 0);

    bop_msg(6,
            "map %s (%p) adding from task %u the range %p, size %zd, obj %p",
            map->name,
            map,
            ppr_id,
            base,
    ...
    char *data =
        obj == NULL ? NULL : data_clone(map->residence, (char *)obj, size);
    ...
    if (!has_overlap) { /* no overlap */
        ...
        if (n->r.base < top->r.base) { /* insert left */
            ...
            //     bop_msg( 3, "n->lc->r.base %p, n->lc->r.size %zd, n->r.base
            //     %p, n->rc->r.base %p", n->lc->r.base, n->lc->r.size,
            //     n->r.base, n->rc->r.base);
            assert(n->lc == NULL ||
                   n->lc->r.base + n->lc->r.size <= n->r.base);
            top->lc = NULL;
        } else { /* insert right */
            n->lc = top;
            n->rc = top->rc;
    ...
    bop_msg(3,
            "Overlap, write_union [%p, %p), new range [%p, %p)",
            target->base,
    ...
    BOP_abort_spec(
        "non-perfect range overlap in post-wait is not implemented");
    return NULL;
...
static char
assert_no_overlap(map_t *map1, map_t *map2)
{
    ...
    if (has_overlap) {
        bop_msg(1,
                "maps %s and %s overlap at %p for %zd bytes",
                map1->name,
                map2->name,
                copy->root->r.base,
                copy->root->r.size);
...
static void
receive_range(void *sum, mem_range_t *range)
{
    channel_t *ch         = (channel_t *)sum;
    mem_range_t *approved = map_add_range_from_task(
        &received, range->base, range->size, range->rec, ch->sender);
    memcpy((void *)approved->base, approved->rec, approved->size);
}

static inline void
wait_until_posted(channel_t *ch)
{
    if (ch->is_posted)
        return;
    int b = 1;
...
channel_wait(addr_t id)
{
    ...
    if (!assert_no_overlap(&ch->data, &read_map) ||
        ...
        bop_msg(1,
                "channel wait abandoned due to a access-before-wait conflict");
        return;
    }
...
static void
mdfy_aftr_pst_chk(mem_range_t *mem)
{
    ...
    char diff =
        memcmp(committed + mem->base - match->base, received, mem->size);

    if (diff) {
        bop_msg(1,
                "modify-after-post by task %u for %p, %zd bytes",
                mem->task,
                mem->base,
                mem->size);
