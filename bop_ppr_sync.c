...
void
partial_group_set_size(int psize)
{
...
int
partial_group_get_size(void)
{
    return sync_data->partial_size;
}

int
ppr_group_get_size(void)
{
    if (sync_data == NULL)
        return 1;
    else
        return sync_data->group_size;
}

void
ppr_sync_data_reset(void)
{
    int i;
...
ppr_group_set_size(int gsize)
{
    ...
    if (sync_data == NULL) {
        ...
        sync_data =
            (sync_data_t *)mspace_calloc(sync_space, 1, sizeof(sync_data_t));
    ...
    if (sync_data->group_size < gsize) {
        sync_data->sync_status = (sync_status_t *)mspace_realloc(
            sync_space, sync_data->sync_status, gsize * sizeof(sync_status_t));
        sync_data->alloc_size = gsize;
    }
...
void
BOP_set_group_size(int size)
{
    ppr_group_set_size(size);
}

void
signal_check_done(int passed)
{
...
void
signal_commit_done(void)
{
    bop_lock_release(&sync_data->sync_status[spec_order].data_committed);
    bop_msg(4,
            "signal commit (%d, partial group_size %d)",
            spec_order,
            partial_group_get_size());
}

void
wait_prior_check_done(void)
{
    assert(spec_order >= 1);
...
void
wait_next_commit_done(void)
{
    bop_msg(4,
            "wait commit (%d, partial_group_size %d)",
            spec_order + 1,
            partial_group_get_size());
...
void
wait_group_commit_done(void)
{
    int i;

    bop_msg(4,
            "wait group commit (%d, partial_group_size %d)",
            spec_order,
            partial_group_get_size());
...
void
signal_undy_conceded(void)
{
    bop_lock_release(&sync_data->undy_conceded);
}

void
wait_undy_created(void)
{
    bop_lock_acquire(&sync_data->undy_created);
}

void
wait_undy_conceded(void)
{
    bop_lock_acquire(&sync_data->undy_conceded);
}
