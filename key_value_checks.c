...
// recording functions
static inline void
record_internal(kv_object_t obj, mem_op op, kv_entry_t new_entry)
{
    get_lock();
    // add to the write/read region
    bop_assert(spec_order > 0 &&
               spec_order < BOP_get_group_size()); // sanity checks
...
If any 2 spec tasks R/W the same KEY pair with different VALUE writes, its a
failure.
The check for this is a little messy because theres lots of loops to check.
*/
static inline int
data_range_eq(data_range read, data_range write)
{
    int index;
...
static inline int
entry_conflicts(kv_entry_t read, kv_entry_t write)
{
    // only care if keys are equal
    return data_range_eq(read->key,
                         write->key) && // accessed the same part of struct
           !data_range_eq(read->value,
                          write->value); // didn't see the same things
}
...
obj_correct()
{
    ...
    for (kv_obj = kv_list; kv_obj != NULL; SET(kv_obj, kv_obj->next)) {
        for (read_index = 0; read_index < gs; read_index++) {
            for (SET(read_data, kv_obj->reads[read_index]); read_data != NULL;
                 SET(read_data, read_data->next)) {
                for (write_index = read_index + 1; write_index < gs;
                     write_index++) {
                    for (SET(write_data, kv_obj->writes[write_index]);
                         write_data != NULL;
                         SET(write_data, write_data->next)) {
...
static inline typeof(lock) *init_obj_lock()
{
    static pthread_mutexattr_t
        attr; // don't want this too die after the process is over
...
static inline void *
copy_range(void *data, size_t size)
{
...
static inline void
complex_data(data_range *data_range, void *data, size_t len)
{
...
static inline void
simple_data(data_range *data_range, int num)
{
...
void
record_str_pr(kv_object_t obj, mem_op op, char *key, char *value)
{
...
void
record_array(
    kv_object_t obj, mem_op op, void *array, int index, size_t array_data_size)
{
    kv_entry_t entry = mspace_malloc(kv_mspace, sizeof(__raw_kv_entry_t));

    // for generic arrays, we store the index (KEY) and the (complex) data,
    // array[index].
...
void
record_str_array(kv_object_t obj, mem_op op, int ind, char *value)
{
    kv_entry_t entry = mspace_malloc(kv_mspace, sizeof(__raw_kv_entry_t));
...
void
record_int_array(kv_object_t obj, mem_op op, int ind, int *array)
{
    record_int_pair(obj, op, ind, array[ind]);
}

void
record_int_pair(kv_object_t obj, mem_op op, int ind, int value)
{
    kv_entry_t entry = mspace_malloc(kv_mspace, sizeof(__raw_kv_entry_t));
...
bop_port_t object_rw_port = {.ppr_check_correctness = obj_correct,
